...
export function registerPenaltyFixture(bus) {
    ...
    `;
        ...
        if (typeof btnOk.addEventListener === 'function') {
        ...
        activeDialog = dialog;
        // Focus
        if (typeof btnOk.focus === 'function') {
            btnOk.focus();
        }
        // Hotkeys
        keyHandler = (e) => {
        ...
        document.addEventListener && document.addEventListener('keydown', keyHandler);
    ...
    function openAcceptDeclineDialog(data) {
        ...
        if (typeof btnAccept.addEventListener === 'function') {
            btnAccept.addEventListener('click', () => choose('accept'));
        }
        else {
            btnAccept.onclick = () => choose('accept');
        }
        if (typeof btnDecline.addEventListener === 'function') {
            btnDecline.addEventListener('click', () => choose('decline'));
        }
        else {
            btnDecline.onclick = () => choose('decline');
        }
        ...
        keyHandler = (e) => {
            ...
            if (e.key === '1') {
                e.preventDefault();
                bus.emit && bus.emit('ui:penalty.fixture.choice', { decision: 'accept', resolution: data.resolution });
                return;
            }
            else if (e.key === '2') {
                e.preventDefault();
                bus.emit && bus.emit('ui:penalty.fixture.choice', { decision: 'decline', resolution: data.resolution });
                return;
            }
            else if (e.key === 'Enter' || e.key === ' ') {
                const el = document.activeElement;
                if (el && el.tagName === 'BUTTON' && typeof el.click === 'function') {
                    e.preventDefault();
                    el.click();
                }
            }
