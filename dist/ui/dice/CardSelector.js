...
export class CardSelector {
    ...
    wireBusListeners() {
        ...
        this.bus.on('ui:playbookSelected', (payload) => {
            if (this.isBroadcasting)
                return;
            this.applyPlaybookSelection(payload.playbook);
        });
        this.bus.on('ui:defensiveCardToggled', (payload) => {
            if (this.isBroadcasting)
                return;
            this.applyDefensiveToggle(payload.cardId);
        });
        this.bus.on('ui:cardSelected', (payload) => {
            if (this.isBroadcasting)
                return;
    ...
    setupPlaybookSelector(container) {
    ...
    setupDefensiveSelector(container) {
    ...
    createDefensiveCardButton(card) {
    ...
    selectPlaybook(playbook) {
        ...
        try {
            this.bus.emit('ui:playbookSelected', { playbook });
        }
        finally {
    ...
    applyPlaybookSelection(playbook) {
        ...
        const allCards = DICE_ENGINE_PLAYS[playbook] || [];
    ...
    toggleDefensiveCard(cardId) {
        this.applyDefensiveToggle(cardId);
        // Emit toggle event for observers (echo for consumers)
        this.isBroadcasting = true;
        try {
            this.bus.emit('ui:defensiveCardToggled', { cardId });
        }
        finally {
            this.isBroadcasting = false;
        }
    }
    applyDefensiveToggle(cardId) {
    ...
    selectCard(cardId) {
        ...
        this.isBroadcasting = true;
        try {
            this.bus.emit('ui:cardSelected', { cardId });
        }
        finally {
            this.isBroadcasting = false;
        }
    ...
    announceToScreenReader(message) {
        ...
        if (!liveRegion) {
            ...
            newRegion.style && (newRegion.style.cssText = 'position:absolute;left:-10000px;width:1px;height:1px;overflow:hidden;');
    ...
    updatePlaybookState(state) {
        this.playbookState = { ...state };
        this.renderPlaybookCards();
    }
    updateDefensiveState(state) {
