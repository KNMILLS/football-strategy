...
export class PenaltyModal {
    ...
    register() {
        ...
        const waitForElements = () => {
            ...
            this.bus.on('ui:penaltyOverride', (payload) => {
    ...
    setupEventListeners(backdrop, modal) {
        // Close button
        const closeButton = modal.querySelector('.dice-modal-close');
        if (closeButton) {
    ...
    showPenaltyModal(data) {
        ...
        const content = this.modalElement.querySelector('#dice-penalty-content');
        const buttons = this.modalElement.querySelector('#dice-penalty-buttons');
        if (!content || !buttons)
            return;
        ...
        if (this.config.accessibility.announceResults) {
            this.announceToScreenReader(`Penalty decision required. ${data.meta.penaltyType} for ${data.meta.yards} yards. ${data.meta.description}`);
        }
    }
    showInformationalModal(penaltyInfo, description) {
        if (!this.modalElement)
            return;
        this.state.isVisible = true;
        const content = this.modalElement.querySelector('#dice-penalty-content');
        ...
        // Announce to screen readers
        if (this.config.accessibility.announceResults) {
            this.announceToScreenReader(`Automatic penalty override. ${description}`);
        }
    }
    handlePenaltyDecision(decision) {
        this.state.isAccepting = decision === 'accept';
        this.state.isDeclining = decision === 'decline';
        // Update button states
        const acceptButton = this.modalElement?.querySelector('.dice-penalty-accept');
        const declineButton = this.modalElement?.querySelector('.dice-penalty-decline');
    ...
    hideModal() {
        ...
        const acceptButton = this.modalElement.querySelector('.dice-penalty-accept');
        const declineButton = this.modalElement.querySelector('.dice-penalty-decline');
        const okButton = this.modalElement.querySelector('.dice-penalty-ok');
        if (acceptButton) {
    ...
    showOverrideModal(description) {
        this.showInformationalModal({ penaltyType: 'Override', yards: 0 }, description);
    }
...
//# sourceMappingURL=PenaltyModal.js.map