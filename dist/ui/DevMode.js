...
export function registerDevMode(bus) {
    ...
    const panel = $('controls-dev');
    const themeSelect = $('theme-select');
    const sfxEnabled = $('sfx-enabled');
    const sfxVolume = $('sfx-volume');
    const playerDeck = $('dev-player-deck');
    const aiDeck = $('dev-ai-deck');
    const possession = $('dev-possession');
    const seedInput = (() => {
    ...
    const validatedBatch = (() => {
        if (!panel || typeof document === 'undefined')
            return { container: null, count: null, chunk: null, pat: null, run: null };
        const container = document.createElement('div');
        container.className = 'control-row';
        const label = document.createElement('label');
        label.textContent = 'Validated Batch:';
        label.style.marginRight = '8px';
        ...
        count.max = '10000';
        count.value = '100';
        count.id = 'validated-batch-count';
        count.title = 'Number of games';
        count.style.width = '6rem';
        count.style.marginRight = '8px';
        const chunk = document.createElement('input');
        chunk.type = 'number';
        chunk.min = '1';
        chunk.max = '1000';
        chunk.value = '50';
        chunk.id = 'validated-batch-chunk';
        chunk.title = 'Chunk size';
        chunk.style.width = '5rem';
        chunk.style.marginRight = '8px';
        const pat = document.createElement('select');
        pat.id = 'validated-batch-pat';
        pat.title = 'Player PAT policy';
        for (const v of ['auto', 'kick', 'two']) {
        ...
        run.id = 'run-validated-batch';
        run.textContent = 'Run Validated Batch';
        container.appendChild(label);
    ...
    if (devToggle) {
        devToggle.addEventListener('change', () => {
            ...
            if (panel) {
                ...
                panel.setAttribute('aria-hidden', (!enabled).toString());
            }
    ...
    bus.on('ui:devModeChanged', ({ enabled }) => {
        try {
            globalThis.GS = globalThis.GS || {};
            globalThis.GS.__devMode = { enabled: !!enabled };
        }
        catch (error) { /* ignore */ }
        if (panel) {
            panel.hidden = !enabled;
    ...
    if (runAutoBtn) {
        runAutoBtn.addEventListener('click', () => {
            ...
            if (isFinite(provided) && provided > 0) {
                bus.emit('qa:runAutoGame', { seed: provided, playerPAT: 'auto' });
            }
            else {
    ...
    if (validatedBatch.run && validatedBatch.count && validatedBatch.pat) {
        validatedBatch.run.addEventListener('click', () => {
            ...
            const pat = (validatedBatch.pat.value === 'kick' || validatedBatch.pat.value === 'two') ? validatedBatch.pat.value : 'auto';
            ...
            bus.emit('qa:runValidatedBatch', { seeds, playerPAT: pat, chunkSize });
        });
    }
