...
try {
    bus.on && bus.on('dev:startGame', () => { isTestGame = true; });
}
catch (error) {
    ...
    try {
        bus.on('ui:newGame', async ({ deckName, opponentName }) => {
            try {
                ...
                const validPlaybook = (name) => (['West Coast', 'Spread', 'Air Raid', 'Smashmouth', 'Wide Zone'].includes(name)) ? name : 'West Coast';
                const playerPlaybook = validPlaybook(deckName);
                // Map opponent to an AI playbook style
                let aiPlaybook = 'West Coast';
                if (opponentName === 'Andy Reid')
                    aiPlaybook = 'Air Raid';
                else if (opponentName === 'Kliff Kingsbury')
                    aiPlaybook = 'Air Raid';
                else if (opponentName === 'Marty Schottenheimer')
                    aiPlaybook = 'Smashmouth';
                else if (opponentName === 'Bill Walsh')
                    aiPlaybook = 'West Coast';
                else if (opponentName === 'Mike Shanahan')
                    aiPlaybook = 'Wide Zone';
                ...
                window.GS_RUNTIME = { flow, playerPlaybook, aiPlaybook, state: stateAfterKO, seed };
                ...
                try {
                    ...
                    console.debug('Kickoff event translation failed', error);
                }
                ...
                try {
                    ...
                    console.warn('PlayCaller personality load failed', error);
                }
                pcPlayer.reset(seed);
                pcAI.reset(seed + 1);
                ...
                bus.on('hudUpdate', ({ possession, down, toGo, ballOn }) => {
                    try {
                        ...
                        console.debug('HUD sync failed', error);
                    }
                });
                    ...
                    try {
                        ...
                        if (currentState.possession === 'player') {
                            ...
                            if (cardId === 'punt') {
                                ...
                                const newState = puntResult.state;
                            ...
                            try {
                                ...
                                try {
                                    window.GS_RUNTIME.__lastOffLabel = String(cardId);
                                ...
                                try {
                                    const playType = (legacyCard && /pass/i.test(legacyCard.label)) ? 'pass' : 'run';
                                    ...
                                    console.debug('Tendency record failed', error);
                                }
                            }
                            catch (error) {
                                console.warn('Engine resolution failed, falling back to deterministic:', error);
                                // Fallback to original system
                                const input = { deckName: rt.playerPlaybook, playLabel: String(cardId), defenseLabel: defPicked.label };
                            ...
                            try {
                                ...
                                console.warn('Engine resolution failed, falling back to deterministic:', error);
                                // Fallback to original system
                                const input = { deckName: rt.aiPlaybook, playLabel: offPicked.playLabel, defenseLabel: cardId };
                                const res = flow.resolveSnap(currentState, input);
                                translate(res.events);
                                window.GS_RUNTIME.state = res.state;
                            }
        ...
        console.warn('Failed to register ui:newGame handler', error);
    }
}
function dispose() {
