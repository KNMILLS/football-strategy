...
export class OutlierDetector {
    ...
    detectTableOutliers(analysis, playbookGroups) {
        ...
        if (peers.length < 3) {
            ...
            const pushAbs = (metric, value, expected, severity) => {
                outliers.push({
            ...
            if (analysis.avgYards > 50)
                pushAbs('avgYards', analysis.avgYards, 50, 'high');
            if (analysis.explosiveRate > 60)
                pushAbs('explosiveRate', analysis.explosiveRate, 60, 'high');
            if (analysis.turnoverRate > 40)
                pushAbs('turnoverRate', analysis.turnoverRate, 40, 'high');
            if (analysis.penaltyRate > 40)
                pushAbs('penaltyRate', analysis.penaltyRate, 40, 'medium');
    ...
    detectZScoreOutliers(analysis, peers) {
        ...
        for (const metric of metrics) {
            const values = peerSet.map(p => p[metric.name]).filter(v => typeof v === 'number');
            if (values.length === 0)
                continue;
    ...
    detectIQROutliers(analysis, peers) {
        ...
        for (const metricName of metrics) {
            const peerSet = peers.filter(p => p !== analysis);
            const values = peerSet.map(p => p[metricName]).filter(v => typeof v === 'number').sort((a, b) => a - b);
            if (values.length < 3)
                continue; // Need at least 3 peer values for IQR
