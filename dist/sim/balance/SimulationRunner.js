...
import { GuardrailChecker } from './GuardrailChecker';
...
export class SimulationRunner {
     ...
     */
    async processBatch(batch, progress, errors) {
        const promises = batch.map(async (table) => {
            try {
                ...
                const analysis = await this.analyzer.analyzeTable(table.id, table.offenseCard, table.defenseCard, table.playbook, this.config.sampleSize);
                return analysis;
            }
            catch (error) {
