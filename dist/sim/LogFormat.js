...
export function formatEventsToLegacyLog(events, ctx) {
    let out = '';
    for (const ev of events) {
        if (ev.type === 'log') {
            ...
            if (kind === 'TD') {
                if (playerDelta === 6)
                    out += line('HOME scores 6 points.');
                if (aiDelta === 6)
                    out += line('AWAY scores 6 points.');
            }
            else if (kind === 'XP') {
                if (playerDelta === 1)
                    out += line('Extra point is good.');
                if (aiDelta === 1)
                    out += line('AI extra point is good.');
                ...
                if (playerDelta === 2)
                    out += line('Safety! HOME is awarded 2 points.');
                if (aiDelta === 2)
                    out += line('Safety! AWAY is awarded 2 points.');
            }
        }
