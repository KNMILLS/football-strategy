...
export function parseCsvContent(csvContent) {
    ...
    try {
        ...
        for (const [tableName, table] of Object.entries(tables)) {
            const hasEntries = Object.keys(table.entries || {}).length > 0;
            if (!hasEntries)
                continue; // keep metadata tables without strict validation
            const validation = MatchupTableSchema.safeParse(table);
            if (!validation.success) {
                warnings.push(`Table '${tableName}' validation warnings: ${validation.error.issues?.length ?? 1} issues`);
            }
        }
    }
    catch (error) {
 ...
 */
function parseCsvRowToObject(row, headers) {
 ...
 */
function processTableMetadataRow(csvRow, rowNumber) {
    ...
    try {
        ...
        const table = {
            ...
            meta: {
                ...
                risk_profile: (['low', 'medium', 'high'].includes(csvRow.meta_risk_profile || '') ? csvRow.meta_risk_profile : 'medium'),
 ...
 */
function processEntryRow(csvRow, tables, rowNumber) {
    ...
    try {
        ...
        const entry = {
            ...
            clock: (['10', '20', '30'].includes(clock || '') ? clock : '20'),
        ...
        if (turnover_type) {
            entry.turnover = {
                ...
                return_to: (turnover_return_to === 'LOS' ? 'LOS' : 'LOS')
            };
        }
