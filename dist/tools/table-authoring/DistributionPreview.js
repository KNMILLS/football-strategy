 ...
 */
export function analyzeDistribution(table, options = {}) {
    const { binSize = 5, showAsciiHistogram = true, analyzeBalance = true, detectPlaybookIdentity = true } = options;
...
function calculateStatistics(outcomes) {
    ...
    const turnoverOutcomes = outcomes.filter(o => o.tags?.some((tag) => String(tag).includes('turnover'))).length;
 ...
 */
function createDistributionBins(outcomes, binSize) {
    const yards = outcomes.map(o => o.yards);
 ...
 */
function analyzeTableBalance(table, outcomes, statistics) {
    const violations = [];
 ...
 */
function detectPlaybookIdentityFromTable(table) {
    const matches = [];
    const suggestions = [];
    ...
    for (const [playbook, identity] of Object.entries(PLAYBOOK_IDENTITY_GUARDRAILS)) {
        ...
        if (table.meta.risk_profile === 'high' && identity.passRate.min > 60)
            score += 1;
        if (table.meta.risk_profile === 'low' && identity.passRate.min < 50)
            score += 1;
        if (score >= 3) {
 ...
 */
function generateAsciiHistogram(bins, statistics) {
 ...
 */
function generateTextSummary(statistics, balance) {
 ...
 */
export function createSimpleChart(table) {
    const outcomes = Object.entries(table.entries).map(([sum, outcome]) => ({
        sum: parseInt(sum),
        yards: outcome.yards
    }));
 ...
 */
export function validateDistributionRequirements(table) {
    const issues = [];
...
//# sourceMappingURL=DistributionPreview.js.map