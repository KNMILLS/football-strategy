 ...
 */
export function validateMatchupTable(table) {
    const errors = [];
    const warnings = [];
    try {
        ...
        for (const [sum, entry] of Object.entries(parsedTable.entries)) {
            ...
            if (entryObj.turnover) {
                if (typeof entryObj.turnover !== 'object' || entryObj.turnover === null) {
                    ...
                    const turnover = entryObj.turnover;
                    if (!['INT', 'FUM'].includes(turnover.type)) {
                    ...
                    if (turnover.return_to !== undefined && !['LOS'].includes(turnover.return_to)) {
 ...
 */
export function validatePenaltyTable(table) {
    const errors = [];
    const warnings = [];
    try {
        // First validate the basic schema
 ...
 */
export function validateTableSet(tables) {
    const errors = [];
    const warnings = [];
    let allValid = true;
...
//# sourceMappingURL=MatchupTableValidator.js.map