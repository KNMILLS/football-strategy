...
export class UIAutomationTester {
    ...
    async testDeckSelection() {
        try {
            ...
            const deckChanged = this.interactionLog.some(interaction => interaction.type === 'deck_selection' && interaction.data?.deckName === 'Ball Control');
            if (!deckChanged) {
    ...
    async testOpponentSelection() {
        try {
            ...
            await new Promise(resolve => setTimeout(resolve, 100));
            // Verify event was emitted
            const opponentChanged = this.interactionLog.some(interaction => interaction.type === 'opponent_selection' && interaction.data?.opponentName === 'Andy Reid');
            if (!opponentChanged) {
    ...
    async testNewGameButton() {
        try {
            ...
            const newGameEvent = this.interactionLog.some(interaction => interaction.type === 'new_game' && interaction.data?.action === 'start');
            if (!newGameEvent) {
     ...
     */
    async testControlResponsiveness() {
        try {
     ...
     */
    async validateGameStateAfterStart() {
        try {
...
//# sourceMappingURL=UIAutomationTester.js.map