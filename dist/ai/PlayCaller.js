...
function isLegal(playLabel, deps, st, playerIsHome) {
...
function softmaxPick(rng, items, temp) {
...
function yardlineText(st, offenseIsHome) {
    const abs = st.ballOn;
    if (offenseIsHome) {
        if (abs <= 50)
            return `OWN ${Math.max(1, abs)}`;
        return `OPP ${Math.max(1, 100 - abs)}`;
    }
    else {
        if (abs >= 50)
            return `OWN ${Math.max(1, 100 - abs)}`;
...
export class PlayCaller {
    deps;
    playerIsHome;
    rng = createLCG(123456789);
    model = new OpponentModel({ halfLife: 12, alpha: 0.5 });
    lastOffenseCalls = [];
    lastDefenseCalls = [];
    personality = {
    ...
    constructor(deps, playerIsHome) {
        this.deps = deps;
        this.playerIsHome = playerIsHome;
    }
    async loadPersonality(cfg) {
        try {
            // Attempt to load JSON config if available at runtime path (bundlers may inline or fail gracefully)
            let fromFile = {};
            try {
                const mod = await import('./playcaller.config.json').catch(() => null);
                fromFile = (mod && (mod.default || mod)) || {};
            }
            catch { }
            this.personality = { ...this.personality, ...fromFile };
        }
        catch { }
        if (cfg)
            this.personality = { ...this.personality, ...cfg };
    }
    reset(seed) {
    ...
    add_observation(pre, offenseLabel, defenseLabel) {
    ...
    choose_offense_play(state, deckName) {
        ...
        const candidates = hand.map(card => {
            ...
            let matchup = 0;
            if (cat === 'run_mid')
                matchup = (+0.2) * (d.passing + d.prevent + d.prevent_deep) + (-0.3) * (d.goal_line + d.short_yd + d.inside_blitz);
            else if (cat === 'run_edge')
                matchup = (+0.15) * (d.passing + d.balanced) + (-0.25) * (d.short_yd + d.outside_blitz);
            else if (cat === 'pass_short')
                matchup = (+0.25) * (d.balanced + d.passing) + (-0.25) * (d.inside_blitz + d.outside_blitz + d.goal_line);
            else if (cat === 'pass_deep')
                matchup = (+0.35) * d.passing + (-0.5) * (d.prevent + d.prevent_deep);
            else if (cat === 'gadget')
                matchup = (+0.15) * (1 - d.short_yd) + (-0.2) * d.goal_line;
            // Context score: field position and down-distance
            let context = 0;
            const short = state.toGo <= 2;
            const long = state.toGo >= 8;
            if (short && (cat === 'run_mid' || cat === 'run_edge' || /keeper|slant|off tackle/i.test(card.label)))
                context += 0.25;
            if (long && (cat === 'pass_short' || cat === 'pass_deep'))
                context += 0.2;
            if (fieldZone === 'own_gl' || fieldZone === 'backed') {
                if (cat === 'pass_deep' || cat === 'gadget')
                    context -= 0.3;
            }
            if (fieldZone === 'red' && state.down === 1 && cat === 'run_mid')
                context += 0.05; // diversify but keep base viable
            // Anti-repeat penalty
            ...
            return { item: { deckName, playLabel: card.label }, util };
        });
        let chosen;
        if (this.rng() < epsilon) {
        ...
        if (this.lastOffenseCalls.length > 8)
            this.lastOffenseCalls.shift();
        ...
        try {
            ...
            if (isDev) {
                ...
                globalThis.GS?.bus?.emit?.('qa:debug', { text: JSON.stringify(telem) });
            }
        }
        catch { }
        return chosen;
    }
    choose_defense_play(state) {
        ...
        const candidates = options.map(label => {
            ...
            let matchup = 0;
            if (cat === 'goal_line' || cat === 'short_yd')
                matchup = (+0.35) * (o.run_mid + o.run_edge) + (-0.25) * o.pass_deep;
            else if (cat === 'inside_blitz' || cat === 'outside_blitz')
                matchup = (+0.25) * (o.pass_short) + (-0.25) * o.gadget;
            else if (cat === 'passing')
                matchup = (+0.35) * (o.pass_short + o.pass_deep) + (-0.2) * (o.run_mid);
            else if (cat === 'prevent_deep')
                matchup = (+0.5) * o.pass_deep + (-0.35) * (o.run_mid + o.run_edge);
            else if (cat === 'prevent')
                matchup = (+0.35) * o.pass_deep + (-0.2) * o.run_mid;
            else
                matchup = (+0.2) * (o.run_mid + o.run_edge + o.pass_short) - 0.05 * o.gadget;
            let context = 0;
            const short = state.toGo <= 2;
            if (short && (cat === 'goal_line' || cat === 'short_yd' || cat === 'inside_blitz'))
                context += 0.2;
            if (fieldZone === 'red' && (cat === 'goal_line' || cat === 'short_yd'))
                context += 0.1;
            if (state.quarter === 4 && state.clock <= 2 * 60 && cat === 'prevent_deep')
                context += 0.25;
        ...
        if (this.lastDefenseCalls.length > 8)
            this.lastDefenseCalls.shift();
        try {
            const isDev = (() => {
                try {
                    if (typeof localStorage !== 'undefined')
...
//# sourceMappingURL=PlayCaller.js.map