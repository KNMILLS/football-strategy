...
function yardsToOppGoal(ballOnFromHome, playerIsHome, possession) {
    const offenseIsHome = (possession === 'player') === playerIsHome;
...
function pickPlayFromHand(ai, desiredType, fallbackType) {
...
function offensivePassProbability(ai) {
...
function decideFourthDown(ai) {
    ...
    if (toGo <= 2 && onOpponentSide)
        goForItViable = true;
    if (toGo <= 4 && yardsToOpp <= 65 && yardsToOpp >= 55 && (coach.aggression + urgencyBoost) > 0.5)
        goForItViable = true; // around opp 45–35
    // Scoring choices
    if (fgViable) {
    ...
    if (goForItViable) {
        ...
        if (label)
            return { kind: 'play', deckName: ai.deckName, playLabel: label };
    }
...
export function chooseOffense(ai) {
    const { state } = ai;
    // Fourth down logic
    if (state.down === 4) {
        ...
        if (fourth) {
            if (fourth.kind === 'fieldGoal') {
                ...
                if (attemptYards <= 65)
                    return fourth; // guard improbable but allow tests to stub
            }
    ...
    if (alt)
        return { kind: 'play', deckName: ai.deckName, playLabel: alt };
...
export function chooseDefense(ai) {
    const { state, rng } = ai;
    const { down, toGo, quarter, clock } = state;
    ...
    if (late && leading && (defenseScore - offenseScore) >= 6) {
        const prefer = ['Prevent Deep', 'Prevent'];
    ...
    if (toGo <= 2)
        ...
        bucket = 'long';
    if (red && (state.toGo <= 3 || state.down === 1))
        bucket = 'short';
    let candidates = [];
    if (bucket === 'short')
        candidates = ['Goal Line', 'Short Yardage', 'Inside Blitz'];
    else if (bucket === 'medium')
        candidates = ['Run & Pass', 'Pass & Run'];
    else
        candidates = ['Passing', 'Outside Blitz'];
    ...
    const weights = pick.map(label => {
        let weight = 1;
        if (bucket === 'short') {
            if (label === 'Goal Line')
                weight = 1.2;
            if (label === 'Short Yardage')
                weight = 1.4;
            if (label === 'Inside Blitz')
                weight = 1.1;
        }
        else if (bucket === 'medium') {
            if (label === 'Run & Pass')
            ...
            if (label === 'Passing')
                weight = 1.3;
            if (label === 'Outside Blitz')
                weight = 1.1;
            if (label === 'Prevent Deep')
                weight = 1.4;
        }
...
//# sourceMappingURL=Playcall.js.map