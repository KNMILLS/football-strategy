...
export function chooseWeighted(rng, items) {
...
export function isRedZone(ballOnFromHome, playerIsHome, possession) {
...
export function yardsToOppGoal(ballOnFromHome, playerIsHome, possession) {
    const offenseIsHomeFlag = (possession === 'player') === playerIsHome;
    return offenseIsHomeFlag ? (100 - ballOnFromHome) : ballOnFromHome;
}
export function offenseIsHome(ai) { return (ai.state.possession === 'player') === ai.playerIsHome; }
export function offenseAbsFromHome(ai) { return offenseIsHome(ai) ? ai.state.ballOn : (100 - ai.state.ballOn); }
export function fieldZoneId(ai) {
...
export function categorizeLabel(label) {
    ...
    if (/\bdraw\b/.test(l))
        return 'draw_screen';
    if (/screen pass/.test(l))
        return 'draw_screen';
    // Deep
    if (/long bomb|stop & go|stop and go|streak|go route|corner|post|deep/.test(l))
        return 'deep_like';
    // Quick game
    if (/flair pass|sideline pass|look in pass|pop pass|button hook|down & out pass|down & in pass|quick pass|slant/.test(l))
        return 'quick_like';
    // Inside/ground (default for runs and RPO)
    return 'inside_like';
}
export function computeCategoryWeights(ai) {
    const { state } = ai;
    ...
    // Baseline
    let w = { inside_like: 0.45, quick_like: 0.30, draw_screen: 0.10, deep_like: 0.10, gadget_like: 0.05 };
...
function passesWhiteSign(ai, playLabel) {
...
export function pickPlayFromHand(ai, desiredType, fallbackType) {
    ...
    try {
        razzleUsedThisDrive = ai.__rd_used__ === ai.state.possession;
    }
    catch { }
    const isRazzle = (label) => /razzle dazzle/i.test(label);
    const primary = hand.filter(c => {
        if (c.type !== desiredType)
            return false;
        if (!passesWhiteSign(ai, c.label))
            return false;
        if (isRazzle(c.label)) {
            if (razzleUsedThisDrive)
                return false;
            if (inOwnRed && !trailingLate)
                return false;
            // Additional guard: avoid trick play on 4th down unless trailing late
            if (fourthDown && !trailingLate)
                return false;
        }
        return true;
    });
    if (primary.length > 0) {
        ...
        if (isRazzle(label)) {
            try {
                ai.__rd_used__ = ai.state.possession;
            }
            catch { }
        }
        return label;
...
export function offensivePassProbability(ai) {
    ...
    if (down === 3) {
        passP += 0.45;
        if (toGo <= 2)
            passP -= 0.35; // keep some run chance on 3rd-and-short
    }
    if (red) {
        if (toGo <= 3)
            passP -= 0.3;
    ...
    if (twoMin && scoreDiff < 0)
        passP += 0.35;
    if (twoMin && scoreDiff > 0 && toGo <= 2)
        passP -= 0.2;
