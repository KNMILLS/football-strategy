...
export function chooseOffenseMixed(ai) {
    const hand = ai.getOffenseHand();
    const eligible = hand.filter(c => (c.type === 'run' || c.type === 'pass') && (() => {
    ...
    const items = eligible.map(c => {
        ...
        if (down === 1) {
            if (/^Power Up Middle$/i.test(c.label))
                weight *= 0.65; // discourage overuse
            if (/^Power Off Tackle$/i.test(c.label))
                weight *= 1.15;
            if (/^End Run$/i.test(c.label))
                weight *= 1.15;
            if (/^(Sideline Pass|Pop Pass|Flair Pass|Down & Out Pass)$/i.test(c.label))
                weight *= 1.10;
        }
        ...
        if (toGo <= 2) {
            if (/^Power Up Middle$/i.test(c.label))
                weight *= 0.75;
            if (/^(QB Keeper|Slant Run|Power Off Tackle)$/i.test(c.label))
                weight *= 1.20;
            if (/^(Pop Pass|Look In Pass|Button Hook Pass)$/i.test(c.label))
                weight *= 1.10; // occasional quick pass
        }
        ...
        if (late && trailing) {
            if (/(Long Bomb|Stop & Go Pass|Corner|Post|Deep)/i.test(c.label))
                weight *= 1.15;
            if (/Screen Pass/i.test(c.label))
                weight *= 1.05; // safe explosive counter occasionally
        }
        // Small jitter
...
export function chooseOffense(ai) {
    ...
    if (mixed)
        return { kind: 'play', deckName: ai.deckName, playLabel: mixed };
