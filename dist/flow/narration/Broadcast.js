export function chooseCommentaryLines(notes) {
    const cleaned = (notes || []).map((n) => String(n).replace(/\s+/g, ' ').trim()).filter(Boolean);
    if (cleaned.length === 0)
        return { pbp: 'Play is underway.', analyst: 'Watch the leverage and pad level here.' };
    if (cleaned.length === 1)
        return { pbp: cleaned[0] || 'Play is underway.', analyst: 'Good fundamentals at the point of attack.' };
    const isFormation = (s) => /\blines up\b/i.test(s);
    const isBookkeeping = (s) => /^\d+(st|nd|rd|th)\s*&|^Now\s/i.test(s);
    const notFormationOrBook = (s) => !(isFormation(s) || isBookkeeping(s));
...
export function buildBroadcastCall(pre, next, input, outcome, deps) {
    ...
    const hash = (() => {
        const mod = pre.ballOn % 3;
        if (mod === 0)
            return 'left hash';
        if (mod === 1)
            return 'right hash';
        return 'between the hashes';
    })();
    const numbersSide = (() => {
        const mod = pre.ballOn % 2;
        return mod === 0 ? 'near side numbers' : 'far side numbers';
    })();
    const formation = (() => {
        if (/shotgun|down &|bomb|screen|sideline|button|look in|pop pass|flair|flare/i.test(off))
            return 'shotgun, three wide';
        if (/keeper|sneak|short yard|goal line|off tackle|slant run|trap|draw/i.test(off))
            return 'under center, I-formation';
        return 'balanced set';
    })();
    const defLook = (() => {
        if (/outside blitz|inside blitz/i.test(def))
            return 'blitz look';
        if (/passing/i.test(def))
            return 'nickel shell, two-high safeties';
        if (/prevent_deep/i.test(def))
            return 'prevent, safeties deep';
        if (/goal line|short yard/i.test(def))
            return 'heavy front, stacked box';
        return 'balanced front';
    })();
    const motion = (() => {
        if (/sideline|down & out|down & in|screen|flair|flare|button/i.test(off))
            return 'slot in short motion';
        return '';
    })();
    ...
    if (!outcome) {
        ...
        if (y > 0) {
            ...
            if (y >= 20) {
                try {
                    deps.quarterStats.explosives += 1;
                }
                catch { }
            }
        }
        else if (y < 0)
            resultPart = ` Swallowed up for ${Math.abs(y)}.`;
        else
            resultPart = ' Stonewalled at the line.';
    }
    else if (outcome.category === 'fumble') {
        resultPart = ' Ball is loose and recovered.';
        try {
            deps.quarterStats.turnovers += 1;
        }
        catch { }
    }
    ...
    const geo = (() => {
        ...
        if (yardsToGoal <= 20)
            return 'inside the red zone';
        if (next.ballOn >= 48 && next.ballOn <= 52)
            return 'just shy of midfield';
        return `${numbersSide}`;
    })();
    ...
    const pbp = (concise
        ...
        : `${downText} from ${spotPhrase}, Q${pre.quarter} ${clock} — ${scoreText}. ${presnap} ${snap} ${reset}${pressure}${crowd}`).trim();
    let analyst = '';
    if (outcome && (outcome.category === 'gain' || outcome.category === 'loss')) {
        const y = outcome.yards || 0;
        if (y >= 8)
            analyst = isRun ? 'Great cut and second-level block opened that lane.' : 'Beat the leverage outside — timing was perfect.';
        else if (y >= 3)
            analyst = isRun ? 'Good pad level; fell forward behind the guard.' : 'On time and on target; easy yards.';
        else if (y > 0)
            analyst = isRun ? 'Patient run, but backers closed fast.' : 'Checkdown; kept it ahead of the sticks.';
        else if (y === 0)
            analyst = 'Front won at the snap — no movement.';
        else
            analyst = 'That was a brick wall up front; linebackers filled every crease.';
    }
    if (!analyst) {
        if (/outside blitz/i.test(def))
            analyst = 'Edge pressure; QB had to speed it up.';
        else if (/inside blitz/i.test(def))
            analyst = 'A-gap heat — protection slid late.';
        else if (/goal line|short yard/i.test(def))
            analyst = 'Heavy front, gaps corked.';
        else if (/passing/i.test(def))
            analyst = 'Light box with umbrella coverage.';
        else if (/prevent/i.test(def))
            analyst = 'Soft shell, keep-it-in-front spacing.';
        else
            analyst = isRun ? 'Line got just enough push.' : 'Protection held just long enough.';
    }
    try {
        const lastTwo = deps.playInDrive >= 2 ? deps.playInDrive : 0;
        if (lastTwo >= 2 && isRun)
            analyst += ' They’ve hit that off-tackle twice; don’t be shocked if they fake it next series.';
    }
    catch { }
    return { pbp, analyst };
}
