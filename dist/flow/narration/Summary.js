export function buildResultSummary(pre, next, outcome, flags, currentPlayLabel, defenseLabel, deps) {
    const notes = [];
    const homeAway = (side) => (side === 'player' ? 'HOME' : 'AWAY');
    ...
    let zone = null;
    if (yardsToGoal <= 10 && pre.down === 1 && pre.toGo >= yardsToGoal)
        zone = 'GoalToGo';
    ...
    if (zone && zone !== last.zone) {
        if (zone === 'GoalToGo')
            notes.push('Goal to go.');
        else if (zone === 'RedZone')
            notes.push('Red zone opportunity.');
        else if (zone === 'NearMid')
            notes.push('Near midfield.');
        else if (zone === 'AcrossMid')
            notes.push('Across midfield.');
        else if (zone === 'BackedUp')
            notes.push('Backed up near own goal line.');
    }
    const attemptYards = yardsToGoal + 17;
    const inRange = attemptYards <= 45;
    if (inRange && pre.down >= 3 && !last.fgInRange)
        notes.push('Within field goal range.');
    ...
    const defNote = (dl) => {
        const pick = (arr) => arr[Math.floor((deps.rng?.() || Math.random()) * arr.length)];
        if (/goal line/i.test(dl))
            return pick(['Stacked front, safeties pinched.', 'Stacked box, gaps corked.']);
        if (/short yardage/i.test(dl))
            return pick(['Tight box, gaps packed.', 'Heavy front, gaps corked.']);
        if (/passing/i.test(dl))
            return pick(['Light box, two-high shell.', 'Nickel spacing, umbrella coverage.']);
        if (/outside blitz/i.test(dl))
            return pick(['Corner fire off the edge.', 'Nickel off the slot.', 'Safety blitz crashing down.']);
        if (/inside blitz/i.test(dl))
            return pick(['Mike shoots the A-gap.', 'Linebacker dog up the middle.']);
        if (/prevent deep/i.test(dl))
            return pick(['Umbrella shell, keep it in front.', 'Prevent deep — nothing behind.']);
        if (/prevent/i.test(dl))
            return 'Umbrella look, keep-it-in-front spacing.';
        if (/run & pass|pass & run|running/i.test(dl))
            return pick(['Balanced shell.', 'Soft quarters, balanced spacing.']);
        return null;
    };
    const dnote = defNote(String(defenseLabel || ''));
    const lastDef = last.defense || null;
    const isBlitz = /blitz/i.test(defenseLabel || '');
    if (dnote && (isBlitz || (defenseLabel || '') !== lastDef))
        notes.push(dnote);
    if (/punt/i.test(playLabel)) {
        ...
        if (/screen/i.test(playLabel))
            notes.push('Quarterback sets up the screen.');
        else if (/button hook/i.test(playLabel))
            notes.push('Quick button hook route.');
        else if (/look in/i.test(playLabel))
            notes.push('Quick slant to the inside.');
        else if (/sideline/i.test(playLabel))
            notes.push('Throws to the sideline.');
        else if (/down & in/i.test(playLabel))
            notes.push('Throws down and in.');
        else if (/down & out/i.test(playLabel))
            notes.push('Throws down and out.');
        else if (/pop pass/i.test(playLabel))
            notes.push('Quick pop pass over the middle.');
        else if (/flair|flare/i.test(playLabel))
            notes.push('Flair pass to the back.');
        else if (/stop & go/i.test(playLabel))
            notes.push('Stop-and-go, going deep.');
        else if (/bomb/i.test(playLabel))
            notes.push('Going deep downfield.');
        else
            notes.push('Quarterback drops back to pass.');
    }
    else if (/keeper|draw|qb/i.test(playLabel)) {
        notes.push('Quarterback keeps it himself and runs.');
    }
    else {
        if (/end run/i.test(playLabel))
            notes.push('Quarterback hands off around the end.');
        else if (/off tackle/i.test(playLabel))
            notes.push('Quarterback hands off off tackle.');
        else if (/slant run/i.test(playLabel))
            notes.push('Quarterback hands off on an inside slant run.');
        else if (/draw/i.test(playLabel))
            notes.push('Quarterback delays and hands off on a draw play.');
        else if (/trap/i.test(playLabel))
            notes.push('Quarterback hands off on a trap play.');
        else if (/run & pass option/i.test(playLabel))
            notes.push('Quarterback reads the option look.');
        else if (/razzle dazzle/i.test(playLabel))
            notes.push('Trick play! Razzle dazzle in the backfield.');
        else
            notes.push('Quarterback hands off to the running back.');
    }
    if (flags.td) {
    ...
    if (outcome.category === 'incomplete') {
        ...
        if (/blitz/i.test(defenseLabel))
            inc = 'Pressure forces an incomplete pass.';
        else if (/prevent deep/i.test(defenseLabel))
            inc = 'Prevent deep coverage breaks up the deep shot.';
        else if (/passing/i.test(defenseLabel))
            inc = 'Tight coverage, pass falls incomplete.';
