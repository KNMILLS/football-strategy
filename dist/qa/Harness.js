...
function sleepFrame() { return Promise.resolve(); }
function getLogText() {
...
function getHandFromDeck(deck, ids) {
...
export function registerQAHarness(bus) {
    ...
    bus.on('qa:startTestGame', async (p) => {
        try {
            ...
            const deckNameFrom = (s) => (s === 'Ball Control' || s === 'Aerial Style' || s === 'Pro Style') ? s : 'Pro Style';
            const playerDeck = OFFENSE_DECKS[deckNameFrom(p.playerDeck)] || OFFENSE_DECKS['Pro Style'];
            const aiDeck = OFFENSE_DECKS[deckNameFrom(p.aiDeck)] || OFFENSE_DECKS['Pro Style'];
            ...
            while (!state.gameOver) {
                ...
                const offDeckName = offenseIsPlayer ? deckNameFrom(p.playerDeck) : deckNameFrom(p.aiDeck);
    ...
    bus.on('qa:runAutoGame', async (p) => {
        try {
            ...
            bus.emit('qa:startTestGame', { seed, playerDeck: 'Pro Style', aiDeck: 'Pro Style', startingPossession: 'player' });
            await sleepFrame();
        }
        catch (e) {
    ...
    bus.on('qa:downloadDebug', async () => {
        try {
            ...
            a.href = url;
            a.download = 'gridiron-debug.json';
