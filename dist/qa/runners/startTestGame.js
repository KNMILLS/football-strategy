...
export async function startTestGame(bus, p) {
    try {
        ...
        const playerDeckName = deckNameFrom(p?.playerDeck) || randomDeck();
        const aiDeckName = deckNameFrom(p?.aiDeck) || randomDeck();
        ...
        while (!state.gameOver) {
            ...
            if (offDecision.kind === 'fieldGoal') {
                ...
                const before = { ...state };
                const res = resolveSnap(state, { deckName: offenseDeckName, playLabel: offDecision.playLabel, defenseLabel: defDecision.label });
                const choice = (res.events || []).find((e) => e.type === 'choice-required' && e.choice === 'penaltyAcceptDecline');
                if (choice && flow?.finalizePenaltyDecision) {
                    const data = choice.data || {};
                    const a = data.accepted;
