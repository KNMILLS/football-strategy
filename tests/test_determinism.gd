...
func determinism_scripted_five_snaps_repro() -> void:
	var test_seed := 123456
	var a1 := _run_scripted_once(test_seed)
	var a2 := _run_scripted_once(test_seed)
	assert(a1 == a2)

func determinism_sequence_repeatable_outcomes() -> void:
	...
	for step in seq:
		...
		snaps1.append({"outcome": o.duplicate(true)})
	# Second run
	sm.set_seed(QA.SEED_MAIN)
	gs.new_session(QA.SEED_MAIN, 1, 1)
	var snaps2: Array = []
	for step in seq:
		...
		rules.assert_state(gs)
		snaps2.append({"outcome": o2.duplicate(true)})
