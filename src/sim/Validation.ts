...
export interface ValidationReport {
  ...
  stats: {
    ...
    logCount: number;
    scoreEvents: number;
    kickoffEvents: number;
  };
}

export function validateEvents(events: FlowEvent[]): ValidationReport {
  const issues: string[] = [];
  let hudCount = 0;
  let logCount = 0;
  let scoreEvents = 0;
  ...
  if (!hasFinal) issues.push('No final event emitted.');
  if (!hasKickoff) issues.push('No kickoff event observed.');

  // Track quarter/clock monotonicity and yardline bounds
  const lastClockByQuarter: Record<number, number> = {};
  let lastQuarter = 1;
  ...
  if (hudCount === 0) issues.push('No HUD updates observed (black-screen risk).');
  if (logCount < 1) issues.push('Too few log lines emitted.');
