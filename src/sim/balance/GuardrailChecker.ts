...
export interface GuardrailViolation {
  ...
  deviation: number; // How far outside the range (positive = above max, negative = below min)
  description: string;
...
export class GuardrailChecker {
   ...
   */
  checkCompliance(analysis: TableAnalysis): ComplianceResult {
   ...
   */
  private checkBalanceGuardrails(
    ...
    this.checkGuardrail(
      ...
      violations,
      warnings,
      'Explosive pass rate significantly impacts scoring balance'
    );
    ...
    this.checkGuardrail(
      ...
      violations,
      warnings,
      'Sack rate affects defensive effectiveness and game flow'
    );
    ...
    this.checkGuardrail(
      ...
      violations,
      warnings,
      'Turnover rate dramatically affects possession balance'
    );
    ...
    this.checkGuardrail(
      ...
      violations,
      warnings,
      'Penalty rate affects game flow and strategic decision-making'
    );
    ...
    this.checkGuardrail(
      ...
      violations,
      warnings,
      'Red zone efficiency affects scoring balance in critical situations'
    );
  }
   ...
   */
  private checkPlaybookIdentity(
    analysis: TableAnalysis,
    violations: GuardrailViolation[],
    warnings: GuardrailViolation[]
  ): void {
    const identity = PLAYBOOK_IDENTITY_GUARDRAILS[analysis.playbook as keyof typeof PLAYBOOK_IDENTITY_GUARDRAILS];
    if (!identity) return;

    // Pass rate check (playbook specific)
    const passRate = 100 - (analysis.explosiveRate * 0.3); // Rough estimate based on play types
    this.checkRange(
      `playbookPassRate_${analysis.playbook}`,
      passRate,
      identity.passRate,
      violations,
      warnings,
      `Playbook pass rate affects ${analysis.playbook} strategic identity`,
      `Adjust play mix to better reflect ${analysis.playbook} philosophy`
    );

    // Average gain check
    this.checkRange(
      `playbookAvgGain_${analysis.playbook}`,
      analysis.avgYards,
      identity.avgGain,
      violations,
      warnings,
      `Average gain should match ${analysis.playbook} offensive efficiency`,
      `Tune yardage distributions to match ${analysis.playbook} style`
    );

    // Explosive rate check (playbook specific)
    this.checkRange(
      `playbookExplosiveRate_${analysis.playbook}`,
      analysis.explosiveRate,
      identity.explosiveRate,
      violations,
      warnings,
      `Explosive play rate should match ${analysis.playbook} risk profile`,
      `Adjust explosive thresholds to match ${analysis.playbook} identity`
    );
  }
   ...
   */
  private checkDistributionShape(
    analysis: TableAnalysis,
    violations: GuardrailViolation[],
    warnings: GuardrailViolation[]
  ): void {
  ...
  private checkClockBalance(
    ...
    [clock10, clock20, clock30].forEach((rate, index) => {
      ...
      if (rate < 20 || rate > 45) {
        const severity: GuardrailViolation['severity'] = rate < 15 || rate > 50 ? 'high' : 'medium';
  ...
  private checkGuardrail(
    ...
    actual: number,
    guardrail: typeof BALANCE_GUARDRAILS[keyof typeof BALANCE_GUARDRAILS],
    analysis: TableAnalysis,
    violations: GuardrailViolation[],
    warnings: GuardrailViolation[],
    context: string
    ...
    if (actual < min) {
      ...
      const severity: GuardrailViolation['severity'] =
        deviation > (max - min) * 0.5 ? 'high' : 'medium';
  ...
  private checkRange(
    ...
    const severity: GuardrailViolation['severity'] =
      Math.abs(deviation) > (max - min) * 0.3 ? 'high' : 'medium';
  ...
  private calculateOverallCompliance(
    ...
    let overall: ComplianceResult['overall'];
    if (criticalCount > 0) {
      overall = 'critical';
   ...
   */
  async checkBatchCompliance(analyses: TableAnalysis[]): Promise<ComplianceResult[]> {
   ...
   */
  getApplicableGuardrails(playbook: string): string[] {
    const guardrails: string[] = [];

    // Add balance guardrails
    guardrails.push(
      'explosivePassRate', 'sackRate', 'turnoverRate', 'penaltyRate',
      'clockBalance', 'redZoneEfficiency'
    );
