import { createLCG, type RNG } from '../../sim/RNG';
...
export async function runChunk(params: RunChunkParams, postProgress: (p: { done: number; total: number; passed: number; failed: number }) => void): Promise<RunChunkResult> {
  const { seeds, total, playerPAT, chunkSize, charts, tk, playerDeck, aiDeck, playerCoach, aiCoach, cancelRequestedRef } = params;

  const policy = buildPolicy();
  let passed = 0, failed = 0; let sumHome = 0, sumAway = 0;
  ...
  for (let start = 0; start < seeds.length; start += chunkSize) {
    ...
    for (const seed of chunk) {
      ...
      try {
        ...
        sumHome += state.score.player; sumAway += state.score.ai;
        const report = validateEvents(events);
        if (report.ok) passed++; else { failed++; failures.push({ seed, issues: report.issues.slice() }); }
      } catch (err) {
        failed++;
        failures.push({ seed, issues: [String((err as any)?.message || err)] });
      }
    }
