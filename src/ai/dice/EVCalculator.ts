...
/**
 * Expected Value calculation result for a play candidate
 */
export interface EVResult {
  /** Average yards gained/lost */
  expectedYards: number;
  /** Probability of turnover (INT or FUM) */
  turnoverProbability: number;
  /** Probability of first down */
  firstDownProbability: number;
  /** Average clock runoff in seconds */
  expectedClockRunoff: number;
  /** Expected points (considering field position and scoring) */
  expectedPoints: number;
  /** Raw utility value for AI decision making */
  utility: number;
...
/**
 * Play candidate for EV calculation
 */
export interface PlayCandidate {
  ...
  /** Defensive play (for simulation) */
  defensePlay: DefensivePlay;
  /** Matchup table for this offense/defense combination */
...
/**
 * Context for EV calculation including game state and scoring model
 */
export interface EVContext {
  /** Current game state */
  gameState: GameState;
  /** Scoring model for points calculation */
  scorePosition: (yards: number, fieldPosition: number) => number;
  /** Field position bonus/penalty */
  fieldPositionValue: (position: number) => number;
}
...
export class EVCalculator {
  private readonly SIMULATION_COUNT = 200; // Standard simulations
  private readonly CI_SIMULATION_COUNT = 100; // Reduced for CI performance
   ...
   */
  private hasValidTables(matchupTable: any, penaltyTable: any): boolean {
   ...
   */
  private calculateEVWithSimulation(
    candidate: PlayCandidate,
    context: EVContext,
    rngSeed: number,
    useCISettings: boolean
  ): EVResult {
    ...
    for (let i = 0; i < simulationCount; i++) {
      const result = resolveSnap(
        candidate.offenseCardId,
        candidate.defensePlay,
        candidate.matchupTable,
        candidate.penaltyTable,
        context.gameState,
        rng
      );
   ...
   */
  private calculateEVHeuristic(candidate: PlayCandidate, context: EVContext): EVResult {
   ...
   */
  private getHeuristicYards(candidate: PlayCandidate, gameState: GameState): number {
   ...
   */
  private getHeuristicTurnoverProbability(candidate: PlayCandidate): number {
    const playType = candidate.offenseCardId.toLowerCase();

    if (playType.includes('pass') && playType.includes('deep')) return 0.15;
    if (playType.includes('pass') && playType.includes('short')) return 0.08;
    if (playType.includes('run')) return 0.05;
   ...
   */
  private getHeuristicClockRunoff(gameState: GameState): number {
    if (gameState.down >= 3) return 20;
    return 30;
  }
  ...
  calculateEVMatrix(
    offenseCardId: string,
    playbook: PlaybookName,
    matchupTables: Map<DefensivePlay, MatchupTable>,
    penaltyTable: PenaltyTable,
    context: EVContext,
    rngSeed: number,
    useCISettings = false
  ): Map<DefensivePlay, EVResult> {
    const results = new Map<DefensivePlay, EVResult>();
   ...
   */
  private calculateUtility(params: {
    expectedYards: number;
    expectedPoints: number;
    firstDownProbability: number;
    turnoverProbability: number;
 ...
 */
export function defaultScorePosition(yardsGained: number, newFieldPosition: number): number {
  if (newFieldPosition >= 100) return 7; // Touchdown
  if (newFieldPosition <= 0) return -2; // Safety

  // Simplified field goal probability (rough approximation)
  const distanceToFG = 100 - newFieldPosition;
  if (distanceToFG <= 40) return 3; // High probability FG range (35-40 yards)
  if (distanceToFG <= 55) return 1.5; // Medium probability (45-55 yards)
  if (distanceToFG <= 70) return 0.5; // Low probability (60-70 yards)
 ...
 */
export function defaultFieldPositionValue(position: number): number {
  // Prefer midfield over goal line situations
  if (position <= 20 || position >= 80) return -0.1; // Bad field position
  if (position >= 40 && position <= 60) return 0.1; // Good field position
  return 0; // Neutral
}
