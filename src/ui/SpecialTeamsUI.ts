...
type DialogId = 'onsideOrNormal'|'puntReturn'|'safetyFreeKick';

type ButtonSpec = { id: string; label: string; hotkey?: string; ariaLabel?: string };
...
class ModalManager {
  ...
  ensureRoot(): HTMLElement | null {
    ...
    if (existing) { this.root = existing; return existing; }
  ...
  showDialog(id: DialogId, opts: ShowOptions, onChoose: (btnId: string) => void): void {
    ...
    wrap.appendChild(h);
    if (opts.message) wrap.appendChild(msg);
    ...
    const keyHandler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter' || e.key === ' ') {
        if (document.activeElement && (document.activeElement as HTMLElement).tagName === 'BUTTON') {
  ...
  closeDialog(): void {
    ...
    // Restore focus
    if (this.lastFocused && (this.lastFocused as HTMLElement).focus) {
      try { (this.lastFocused as HTMLElement).focus(); } catch {}
    }
    this.lastFocused = null;
  }
}

export function registerSpecialTeamsUI(bus: EventBus): void {
  const modal = new ModalManager();

  // We rely on index.ts forwarding to a ui-level event name
  const anyBus = bus as any;
  if (anyBus.on) {
    anyBus.on('flow:choiceRequired', (payload: any) => {
      const kind: DialogId = payload.choice;
      const data = payload.data || {};

      // Only open if human must choose
      if (kind === 'onsideOrNormal') {
        if (data.kicking !== 'player') return;
        ...
        if (data.receiving !== 'player') return;
