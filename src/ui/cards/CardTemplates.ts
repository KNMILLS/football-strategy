...
export class CardTemplateBuilder {
  ...
  constructor(dimensions: CardDimensions, styles: CardStyles) {
   ...
   */
  createTitleSection(title: string, x: number, y: number, maxWidth: number): string {
    return `
        <!-- Card title -->
   ...
   */
  createDescriptionSection(description: string, x: number, y: number, maxWidth: number, maxHeight: number): string {
    ...
    return `
        ...
        <text id="card-description" x="${x}" y="${y}" font-family="Arial, sans-serif"
          ...
          ${lines.map((line, index) =>
            `<tspan x="${x}" dy="${index === 0 ? 0 : lineHeight}">${this.truncateText(line, maxWidth, 14)}</tspan>`
          ).join('')}
        </text>
    `;
  }
   ...
   */
  createRiskIndicator(riskLevel: number, x: number, y: number): string {
   ...
   */
  createPerimeterBadge(perimeterBias: number, x: number, y: number): string {
   ...
   */
  private wrapText(words: string[], maxWidth: number, fontSize: number): string[] {
...
export class CardTemplateFactory {
  private builder: CardTemplateBuilder;
