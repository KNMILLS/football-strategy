...
import type { VisualPlaybookCard, VisualDefensiveCard, CardRenderConfig } from './types';
...
export class ProgressiveCardSystem {
   ...
   */
  private setupProgressiveEnhancements(): void {
  ...
  private enableWebGLFeatures(): void {
    (this.bus as any).emit('cards:webglEnabled', {});
   ...
   */
  private enableAnimationFeatures(): void {
    (this.bus as any).emit('cards:animationsEnabled', {});
  ...
  private setupCanvasFallback(): void {
    (this.bus as any).emit('cards:fallbackActivated', {
      from: 'svg',
      to: 'canvas',
   ...
   */
  private setupTextFallback(): void {
    (this.bus as any).emit('cards:fallbackActivated', {
      from: 'canvas',
      to: 'text',
   ...
   */
  private handleProgressiveEnhancement(payload: { type: string; enabled: boolean }): void {
    switch (payload.type) {
      case 'webgl':
   ...
   */
  async renderCard(cardId: string): Promise<string> {
   ...
   */
  private renderCardToCanvas(card: VisualPlaybookCard | VisualDefensiveCard): string {
   ...
   */
  private renderCardAsText(card: VisualPlaybookCard | VisualDefensiveCard): string {
   ...
   */
  getCardDefinitions(): CardDefinitions {
